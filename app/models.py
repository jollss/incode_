...
class ResultsIne(Base):
    ...
    status_ine_loads = Column(
        Integer
    )  # 0-no ha cargado imagnes/1-cargo imagenes a QB/2-cargo imagenes-spearow-F/3-cargo imagenes-spearow-B/4-cargo actividad/9-sin datos para enviar
...
class actividad_economica_sub_cat(Base):
    ...
    sub_nombre_economica = Column(String(255))
    sub_clave_economica_id = Column(String(20))
    sub_cat_actividad_economica_id = Column(
        Integer, ForeignKey("cat_actividad_economica.id")
    )
    created_at = Column(DateTime, nullable=True, default=datetime.datetime.now(tz=tz))

    def __init__(
        self,
        sub_nombre_economica,
        sub_clave_economica_id,
        sub_cat_actividad_economica_id,
    ):
...
class HookLog(Base):
    ...
    def get_ids(self):
        ids = namedtuple(
            "ids", "create update process_id account_id validation_id check_id"
        )
