...
def fill_score(session_id):
    ...
    db_session.commit()
    scores = Score(session_id=session.id, id_validation=id_validation_id.id, curp_verification=curp_verification_id.id, id_ocr_confidence=id_ocr_confidence_id.id, value=value, status=status)
    db_session.add(scores)
    db_session.commit()
    

def validate_score(session_id):
    session = db_session.query(Session).filter(Session.session_id == session_id, Session.status == 'ONBOARDING_FINISHED').one()
    if float(session.score.overall_value) > 70 and session.score.overall_status == 'OK':
        images = session.get_images()
        mewtwo_progress_pld.delay(session.user_id, images.front, images.back)
